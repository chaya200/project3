...
function Login(): JSX.Element {
    ...
    const usersMap = (user_name:string,password:string) => {
        let response = false;
        users.map((user: { user_name: string; password: string; id:number }) => {
            if (hash(user_name)===user.user_name && hash(password)===user.password){
                // //get the followed list from the current user
                // let followedList=JSON.parse(JSON.stringify(user.followed_list));
                //send to the localstorage the followed list of the specific user
